...
Writes, next to sponsors.txt:
    sponsor_counts.json           {normalized_name: approvals}   <- consumed today
    sponsor_counts_by_state.json  {normalized_name: {ST: approvals}}
...
OUT_STATE = "sponsor_counts_by_state.json"
...
def read_hub_csvs(directory, years):
    """Aggregate approvals per USCIS employer spelling. Returns (totals, by_state, seen_years).
        ...
        with open(path, newline="", encoding="utf-8", errors="ignore") as f:
            ...
            for row in reader:
                ...
                if not key:
                    continue
                totals[key] += n
                st = (row.get(st_c) or "").strip().upper() if st_c else ""
                if len(st) == 2:
                    by_state[key][st] += n
    ...
    return totals, by_state, seen_years
...
def resolve(names, totals, by_state):
    ...
    for raw in sorted(names):
        ...
        for k in matches:
            st.update(by_state.get(k, {}))
        if st:
            extra_state[key] = dict(st)
    ...
    return extra, extra_state, report, examples
...
def main():
    ...
    totals, by_state, seen = read_hub_csvs(directory, years)
    if not totals:
    ...
    names = our_universe()
    extra, extra_state, report, examples = resolve(names, totals, by_state)
    ...
    out = dict(totals)
    out.update(extra)
    out_state = {k: dict(v) for k, v in by_state.items()}
    out_state.update(extra_state)

    with open(OUT, "w", encoding="utf-8") as f:
        json.dump(out, f, separators=(",", ":"), sort_keys=True)
    with open(OUT_STATE, "w", encoding="utf-8") as f:
        json.dump(out_state, f, separators=(",", ":"), sort_keys=True)

    tiers = collections.Counter()
    for n in out.values():
        tiers["high" if n >= 1000 else "medium" if n >= 100 else "low"] += 1
    print("\nWrote %s (%d keys, %.1f MB) and %s (%d keys)."
          % (OUT, len(out), os.path.getsize(OUT) / 1e6, OUT_STATE, len(out_state)))
