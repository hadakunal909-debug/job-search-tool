...
flowchart TB
  ...
  subgraph SCH["&#9719; scheduled"]
    S["<b>scraper/__init__.py</b><br/>9,318 lines · 38 ATS adapters<br/>1,192 boards"]
    J["score_jobs.py · 2,155 lines"]
  end
  subgraph CLI["&#9723; browser"]
    E["<b>extension/</b><br/>10 files · 15 /api/ext/* routes"]
    A["static/app.js<br/>the client feed"]
  end
  SPINE["<b>THE SPINE</b> — imported by all three<br/>core.py · 3,865 lines · 33 sections<br/>db.py · 2,929 lines · four backends"]
  REQ --> SPINE
  SCH --> SPINE
  CLI --> SPINE
...
flowchart TB
  ...
  SRC --> JD
  G0{"already known"}
  D0["already known<br/><i>:9079</i>"]
  JD --> G0
  G0 -->|dropped| D0
  class D0 trap
  G1{"blocked company"}
  D1["blocked company<br/><i>:9085</i>"]
  G0 --> G1
  G1 -->|dropped| D1
  class D1 trap
  G2{"off-target function title"}
  D2["off-target function title<br/><i>:9107</i>"]
  G1 --> G2
  G2 -->|dropped| D2
  class D2 trap
  G3{"no matching role keyword"}
  D3["no matching role keyword<br/><i>:9108</i>"]
  G2 --> G3
  G3 -->|dropped| D3
  class D3 trap
  G4{"non-US location"}
  D4["non-US location<br/><i>:9117</i>"]
  G3 --> G4
  G4 -->|dropped| D4
  class D4 trap
  G5{"posted over MAX_AGE_DAYS days ago (AGE_LONG_DAYS for long-lived boards)"}
  D5["posted over MAX_AGE_DAYS days ago (AGE_LONG_DAYS for long-lived boards)<br/><i>:9132</i>"]
  G4 --> G5
  G5 -->|dropped| D5
  class D5 trap
  G6{"no federal sponsor record (aggregator)"}
  D6["no federal sponsor record (aggregator)<br/><i>:9153</i>"]
  G5 --> G6
  G6 -->|dropped| D6
  class D6 trap
  G7{"aggregator copy of a job we hold"}
  D7["aggregator copy of a job we hold<br/><i>:9169</i>"]
  G6 --> G7
  G7 -->|dropped| D7
  class D7 trap
...
flowchart TB
  ...
  C["<b>the client feed</b><br/>static/app.js::matches()<br/><i>line 825</i>"]
  ...
  S --- GUARD
  C --- GUARD
  D["<b>the email digest</b><br/>core.py::prefs_match<br/><i>line 2927 — shares the filters, skips \"posted within\"</i>"]
  GUARD -.-> D
  classDef web fill:#e0e4fe,stroke:#4f46e5,color:#101319
  classDef client fill:#e4e7ec,stroke:#5f6573,color:#101319
  classDef sched fill:#cfe8e3,stroke:#0f766e,color:#101319
