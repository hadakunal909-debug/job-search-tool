...
def main():
    ...
    # Informational, not a failure: app.js reads the UTC date and web.py the local one, so these
    # can legitimately differ for a few hours a day. Worth seeing, never worth failing CI over.
    src = io.open(APP_JS, encoding="utf-8").read()
    if "toISOString" in js_function(src, "dateCutoff"):
        print("\nnote: dateCutoff() is UTC-based client-side and local-date server-side; the "
              "cutoff\n      can differ by one day around midnight UTC. Pre-existing, untouched "
              "here.")
