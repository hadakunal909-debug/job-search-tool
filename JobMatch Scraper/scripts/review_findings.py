...
RANKING. Certified H-1B filings first (a count, descending), then STEM-OPT evidence, then how
many postings the employer has run. Deliberately NOT a single score: the inputs are a count and
a boolean, so combining them would hide which of the two fired, and this sheet exists to be read
rather than sorted on. See core.VISA_TAG_LABELS -- `stem_opt` means the employer is E-Verify
enrolled, which is the real STEM-OPT prerequisite, and absence is NOT evidence of a non-sponsor.
...
COLS = ["company", "postings", "runs", "first_run", "last_run", "h1b_filings", "visa_routes",
        "board_url", "ats_type", "sources", "example_url"]
...
def rank(recs):
    return sorted(recs, key=lambda g: (-int(g.get("h1b_filings") or 0),
...
def main():
    ...
    print("%-34s %5s %4s %8s  %-26s %s" % ("EMPLOYER", "POSTS", "RUNS", "H1B", "ROUTES", "BOARD"))
    for g in shown:
        print("%-34s %5d %4d %8s  %-26s %s" % (
            (g["company"] or "")[:34], g["postings"], g["runs"],
            g["h1b_filings"] or "-", (g["visa_routes"] or "-")[:26],
            (g["board_url"] or "-")[:44]))
