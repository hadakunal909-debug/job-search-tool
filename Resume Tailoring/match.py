...
It ranks your résumés and stories against a job using:
  1. IDF-weighted keyword coverage (ats.py) — the base relevance.
...
The IDF itself self-trains: every job you tailor against updates the document-frequency
table, so keyword importance reflects the corpus you actually apply to.
"""
import math
...
def idf_from_model(model):
    """Turn the running document-frequency table into an idf dict (same formula as a corpus
    build). Empty model -> None, so ats falls back to flat weights."""
    df = model.get("df") or {}
    n = int(model.get("n") or 0)
    if not df or n <= 0:
        return None
    return {t: round(math.log((n + 1) / (c + 1)) + 1.0, 4) for t, c in df.items()}
...
def _coverage(text, analyzed):
    score, have, missing = ats.score_against((text or "").lower(), analyzed)
